...
public:
	...
	// Energy threshold (w.r.t. the vacuum level) above or below which particles
	// must be terminated.
	real _min_energy;
	real _max_energy;
...
protected:
	...
	//Ad Hoc Variables for Surface Diffusion/Adsorption
	int _current_primary; //Electron Counter (single_thread)
	int _previous_primary;
	bool _adsorption;
