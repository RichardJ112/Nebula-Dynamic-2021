...
# Point exposure for Linear Depositions
...
voxel_size_pm = int(voxel_size*1000) # voxel size in pm

# Parameters Pri
num = 21 # number of pillars(odd)
...
line_pitch_list = [0.8] #space between points nm
#line_pitch_list = np.around(np.linspace(0,5,num=51),decimals=1) #space between points nm
dep_strat = "seq_l" #seq_p (sequential points), seq_l (sequential lines) 
...
if dep_strat == "seq_p":
    for i in range(total_runs):
        ...
        #title creation
        title_pri = str(int(energy/1000))+"keV_"+str(num)+"_"+str(int(N/1000))+"kpp_pitch_"+str(line_pitch_pm)+"_"+dep_strat+"_"
...
if dep_strat == "seq_l":
    for i in range(total_runs):
        ...
        for i in range(seq_lines):
            for j in numrange:
                starty = j*line_pitch+y
                xj = np.random.normal(x, sigma_beam, N_seq)
                yj = np.random.normal(starty, sigma_beam, N_seq)
                x_p.append(xj)
                y_p.append(yj)

        for j in range(num*seq_lines):
            inputx[j*N_seq:N_seq*(j+1)] = x_p[j]
            inputy[j*N_seq:N_seq*(j+1)] = y_p[j]
